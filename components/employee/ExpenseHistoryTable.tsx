        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-text-secondary">No expenses found.</td></tr>
            ) : (
              filtered.map((expense) => (
                <tr key={expense.id} className="border-t border-border/60 hover:bg-bg-secondary/30 transition-colors">
                  <td className="px-4 py-3 font-medium text-text-primary">{expense.category}</td>
                  <td className="px-4 py-3 text-text-secondary max-w-xs truncate">{expense.description || '—'}</td>
